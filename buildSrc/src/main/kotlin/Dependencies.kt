package com.example.androidbase.presentation

import org.gradle.api.artifacts.dsl.DependencyHandler


object Libs {

    object Android {
        const val NAVIGATION_VERSION = "2.8.0"
        const val ANDROIDX_APPCOMPAT = "androidx.appcompat:appcompat:1.7.0"
        const val ANDROIDX_CONSTRAINT_LAYOUT = "androidx.constraintlayout:constraintlayout:2.1.4"
        const val ANDROIDX_CORE_KTX = "androidx.core:core-ktx:1.13.1"
        const val ANDROIDX_FRAGMENT = "androidx.fragment:fragment-ktx:1.8.3"
        const val VIEW_MODEL = "androidx.lifecycle:lifecycle-viewmodel-ktx:2.8.5"
        const val LIVE_DATA = "androidx.lifecycle:lifecycle-livedata-ktx:2.8.5"
        const val NAVIGATION_FRAGMENT = "androidx.navigation:navigation-fragment-ktx:$NAVIGATION_VERSION"
        const val NAVIGATION_UI = "androidx.navigation:navigation-ui-ktx:$NAVIGATION_VERSION"
        const val SWIPE_REFRESH_LAYOUT = "androidx.swiperefreshlayout:swiperefreshlayout:1.1.0"
    }

    object Google {
        const val ANDROID_MATERIAL = "com.google.android.material:material:1.11.0"
        const val DAGGER_HILT = "com.google.dagger:hilt-android:2.49"
        const val DAGGER_HILT_COMPILER = "androidx.hilt:hilt-compiler:1.1.0"
        const val DAGGER_HILT_ANDROID_COMPILER = "com.google.dagger:hilt-android-compiler:2.49"
    }

    object Retrofit {
        const val RETROFIT = "com.squareup.retrofit2:retrofit:2.11.0"
        const val RETROFIT_INTERCEPTOR = "com.squareup.okhttp3:logging-interceptor:5.0.0-alpha.2"
        const val RETROFIT_CONVERTER_MOSHI = "com.squareup.retrofit2:converter-moshi:2.11.0"
        const val RETROFIT_CONVERTER_GSON = "com.squareup.retrofit2:converter-gson:2.11.0"
    }

    object Room {
        const val ROOM = "androidx.room:room-runtime:2.6.1"
        const val ROOM_COMPILER = "androidx.room:room-compiler:2.6.1"
        const val ROOM_KOTLIN_EXTENSION = "androidx.room:room-ktx:2.6.1"
    }

    object Glide {
        const val GLIDE = "com.github.bumptech.glide:glide:5.0.0-rc01"
        const val GLIDE_COMPILER = "com.github.bumptech.glide:compiler:5.0.0-rc01"
    }

    object Testing {
        const val TEST_JUNIT = "androidx.test.ext:junit:1.1.4"
        const val JUNIT_JUNIT = "junit:junit:4.13.2"
        const val TEST_ESPRESSO = "androidx.test.espresso:espresso-core:3.6.1"
    }
}

fun DependencyHandler.lifecycle() {
    implementation(Libs.Android.VIEW_MODEL)
    implementation(Libs.Android.LIVE_DATA)
    implementation(Libs.Android.ANDROIDX_FRAGMENT)
}

fun DependencyHandler.room() {
    implementation(Libs.Room.ROOM)
    implementation(Libs.Room.ROOM_KOTLIN_EXTENSION)
    kapt(Libs.Room.ROOM_COMPILER)
}

fun DependencyHandler.retrofit() {
    implementation(Libs.Retrofit.RETROFIT)
    implementation(Libs.Retrofit.RETROFIT_CONVERTER_GSON)
    implementation(Libs.Retrofit.RETROFIT_CONVERTER_MOSHI)
    implementation(Libs.Retrofit.RETROFIT_INTERCEPTOR)
}

fun DependencyHandler.daggerHilt() {
    implementation(Libs.Google.DAGGER_HILT)
    kapt(Libs.Google.DAGGER_HILT_ANDROID_COMPILER)
    kapt(Libs.Google.DAGGER_HILT_COMPILER)
}

fun DependencyHandler.navigationComponent() {
    implementation(Libs.Android.NAVIGATION_FRAGMENT)
    implementation(Libs.Android.NAVIGATION_UI)
}

fun DependencyHandler.glide() {
    implementation(Libs.Glide.GLIDE)
    annotationProcessor(Libs.Glide.GLIDE_COMPILER)
}

